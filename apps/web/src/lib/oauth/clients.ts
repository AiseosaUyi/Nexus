...
export async function registerClient(db: SupabaseClient, input: RegisterClientInput): Promise<RegisterClientResult> {
...
export async function getClient(db: SupabaseClient, clientId: string): Promise<OAuthClient | null> {
  const { data } = await db.from('oauth_clients').select('*').eq('id', clientId).maybeSingle();
  if (!data) return null;
